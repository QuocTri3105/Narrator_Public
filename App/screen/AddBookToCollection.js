...
import {addBook, editBook, DetailCollection, editCollection} from "../redux/bookSlice";
import uuid from "react-native-uuid";
import Feather from "react-native-vector-icons/Feather";
import TempAvatar from "../components/TempAvatar";

const AddBookToCollectionScreen = props => {
    const {id, name = '', data} = props.route.params
    const books = useSelector((state) => state.books.books)
    const filteredData = books.filter((item) => {
        const itemExistsInData2 = data.filter((value) => value.id === item.id).length > 0;
        return !itemExistsInData2;
    });
    ...
    const handleDetailCollection = () => {
        if (listSelected.length === 0) {
            ...
            dispatch(editCollection({id, collectionName: name, listCollection: [...data, ...listSelected]}))
    ...
    return (<SafeAreaView forceInset={{top: 'always'}} style={styles.container}>
        ...
        />
        <TouchableOpacity onPress={handleDetailCollection} style={{
...
export default AddBookToCollectionScreen
